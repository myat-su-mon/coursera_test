...
<head>
<meta charset="UTF-8">
<title>Core Subject Update</title>
<style type="text/css">
#spacing {
	margin-top: 10px;
	margin-bottom: 10px;
...
<body>
	...
	<main id="main">
		<section class="about" data-aos="fade-up">
			<div class="container">
				<form:form action="coresubupdate" method="post"
					modelAttribute="coresub">
					<h4 align="center">Update Core Subject</h4>
					<div class="row">
						<div class="col-lg-6">
							<table>
								<tr>
								...
								<tr>
									...
									<td colspan="2"><form:select class="form-control"
											id="spacing" path="subcategory">
											<option value="1">Core - Foundation</option>
											<option value="2">Core - Skill and Knowledge</option>
											<option value="3">Core - National and
												International Perspectives</option>
											<option value="4">Core - Supporting Skills</option>
										</form:select></td>
								</tr>
								<tr>
									<td>Semester</td>
									...
									<table class="table table-striped table-responsive-md">
										<c:forEach var="i" items="${majorsubjects}" varStatus="status">
											<c:if test="${(status.index>=0) and (status.index<15)}">
												<tr>
													<td><form:checkbox path="prereq"
															value="${i.viewSubCode}" /> ${i.viewSubCode}
														[${i.viewSemester} - ${i.viewCredit}]</td>
												</tr>
											</c:if>
										</c:forEach>
									</table>
					...
					<div class="text-center">
						<button class="button" type="submit">New Core Subject</button>
					</div>
				</form:form>
			</div>
		</section>
